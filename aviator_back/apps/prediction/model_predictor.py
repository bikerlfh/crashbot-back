...
class ModelPredictor:
    ...
    def evaluate(self, *, data: list[int]) -> AverageInfo:
        ...
        for key, value in self.average_info.categories_data.items():
            ...
            dict_value.average_bets = round(
                (dict_value.correct_bets / dict_value.count) * 100, 2
            )
            dict_value.average_predictions = Decimal(dict_value.average_predictions)
            dict_value.average_bets = Decimal(dict_value.average_bets)
            sum_average_predictions += dict_value.average_predictions
            sum_average_bets += dict_value.average_bets
            self.average_info.categories_data[key] = dict_value
        self.average_info.average_predictions = round(sum_average_predictions / count_categories, 2)
        self.average_info.average_predictions = Decimal(self.average_info.average_predictions)
        self.average_info.average_bets = round(sum_average_bets / count_categories, 2)
        self.average_info.average_bets = Decimal(self.average_info.average_bets)
