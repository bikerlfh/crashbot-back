...
def send_message_to_user(*, message: str, customer_id: int, **_kwargs):
    customer_data = customers_selectors.filter_customer(
        id=customer_id
    ).values('phone_number').first()
    if not customer_data:
        raise WSCustomerNotFound(f"customer {customer_id} not found")
    telegram_services.send_telegram_message(
        chat_id=customer_data["phone_number"],
        message=message
    )
...
def send_message(message: SocketMessage) -> None:
    ...
    functions = [obj for obj in frame.f_back.f_locals.values() if inspect.isfunction(obj)]
