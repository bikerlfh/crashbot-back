...
class AbstractBaseModel(abc.ABC):
    ...
    def _split_data_to_train(
        ...
        X = np.array(  # NOQA
            [
                data[i : i + self.seq_len]  # noqa
                for i in range(len(data) - self.seq_len)
            ]
        )
        y = np.array(data[self.seq_len :])  # noqa
        return X, y
    ...
    def evaluate(
        ...
        *,
        ...
        @param probability_to_eval: the minimum probability
            of prediction to be considered
        @return: AverageInfo
        """
        probability_to_eval = (
        ...
        y_multiplier = np.array(multipliers[self.seq_len :])  # noqa
